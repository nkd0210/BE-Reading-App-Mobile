...
export class ChapterService {
  ...
  async createChapter(bookId: string, createChapterDto: CreateChapterDto): Promise<Chapter> {
  ...
  async getAllChaptersOfBook(bookId: string, page: number, limit: number): Promise<any> {
    const skip = (page - 1) * limit;

    const allChapters = await this.chapterModel.find({ book: bookId })
  ...
  async getSingleChapter(chapterId: string): Promise<Chapter> {
    const findChapter = await this.chapterModel.findById(chapterId).populate('book').exec();
    if (!findChapter) {
  ...
  private paginateContent(content: string, wordsPerPage: number = 1000): string[] {
  ...
  async getChapter(chapterId: string, pageNumber: number = 1): Promise<any> {
    ...
    if (pageNumber < 1 || pageNumber > totalPages) {
      throw new HttpException(`Invalid page number. Must be between 1 and ${totalPages}`, HttpStatus.BAD_REQUEST);
    }

    const currentPageContent = contentPages[pageNumber - 1];
    const wordCount = currentPageContent.split(/\s+/).filter(word => word.length > 0).length;
